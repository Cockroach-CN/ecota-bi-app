...
import { Tabs, Icon, Toast, Carousel } from "antd-mobile";
...
class Index extends React.Component {
  constructor(props) {
    super(props);
    const { gkey, init } = props.options;
    const settingTabs = (groups.filter(g => g.key === gkey)[0] || { tabs: [] })
      .tabs;
    const tabs = settingTabs.map((tab, i) => ({
      key: String(tab.key),
      title: tab.name
    }));
    this.state = {
    ...
    return (
      <div id="container">
        ...
        <Filter
          ...
          onOk={opts => {
            setOptions({ opts }, () =>
              this.setState({ showModal: false, random: Math.random() })
            );
          }}
          ...
          >
            {(data.charts || []).map((o, ci) => (
              <div key={ci} className="content-chart">
                <div id={`chart-${di}-${ci}-${unionkey}`} />
                {ci === 0 && (
                  <i
                    style={styleFangda}
                    className="iconfont icon-fangda"
                    onClick={() => onLarge((data.charts || [])[ci])}
                  />
                )}
              </div>
            ))}
